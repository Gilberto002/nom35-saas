      ...
      <div class="admin-sidebar-footer">
        <div class="admin-user">
          ...
          <div>
            ...
            <span class="admin-user-role">admin@nom035.mx</span>
          </div>
        </div>
      </div>
    </aside>
            ...
            <table class="companies-table">
              ...
              <tbody>
                <tr v-for="company in filteredCompanies" :key="company.id" class="company-row">
                  ...
                  <td>
                    <span class="plan-chip" :class="`plan-${company.plan}`">
                  ...
                  <td>
                    <div class="co-actions">
                      <button class="co-btn view" @click="viewCompany(company)" title="Ver detalle">
                      ...
                      <button class="co-btn edit" @click="openCompanyModal(company)" title="Editar">
                      ...
                      <button v-if="company.status === 'active'"
                        class="co-btn suspend" @click="toggleCompanyStatus(company)" title="Suspender">
                      ...
                      <button v-else
                        class="co-btn activate" @click="toggleCompanyStatus(company)" title="Activar">
        ...
        <div v-if="currentView === 'plans'" class="view-plans">
          ...
          <div class="plans-grid">
            <div v-for="plan in plans" :key="plan.id"
              class="plan-card" :class="{ featured: plan.slug === 'profesional' }">
          ...
          <div class="modal-box">
            ...
            <div class="modal-body">
              ...
              <div class="mf-field">
                ...
                <textarea v-model="companyForm.notes" placeholder="Notas sobre esta empresa..." class="mf-textarea" rows="2"></textarea>
              </div>
            </div>
...
// ── Demo data ──
const saasStats = ref({
  mrr: 28750, mrrGrowth: 18,
  activeCompanies: 22, totalEmployees: 1840, totalEvaluations: 47,
})
...
const plans = ref([
  { id:1, name:'Básico',      slug:'basico',      price:499,  max_employees:25,  features:['Hasta 25 empleados','Guías I, II y III','3 reportes PDF/mes','Soporte email'],           active_companies:8  },
  { id:2, name:'Profesional', slug:'profesional', price:1299, max_employees:100, features:['Hasta 100 empleados','Reportes ilimitados','Dashboard avanzado','Soporte prioritario'],  active_companies:11 },
  { id:3, name:'Empresarial', slug:'empresarial', price:2999, max_employees:500, features:['Hasta 500 empleados','Acceso API REST','Gestor dedicado','SLA 99.9%'],                   active_companies:3  },
])
...
const filteredCompanies = computed(() => {
  ...
  if (companySearch.value) {
    ...
    data = data.filter(c => c.name.toLowerCase().includes(q) || c.rfc?.toLowerCase().includes(q) || c.email?.toLowerCase().includes(q))
  }
  return data
})
...
function viewCompany(company) {
  showToast(`Detalle de ${company.name} — próximamente`)
}
...
function editPlan(plan) {
  showToast(`Editor de plan ${plan.name} — próximamente`)
}
...
onMounted(() => {
  // loadAdminData() — conecta a /admin/api/...
})
</script>
...
.admin-nav-icon :deep(svg) { width: 16px; height: 16px; }
...
/* MRR banner */
.mrr-banner {
  background: linear-gradient(130deg, #1E1B4B, #312E81, #4338CA);
...
.akpi-icon :deep(svg) { width: 18px; height: 18px; }
...
.filter-sel {
  ...
  background: #13131F url("data:image/svg+xml,%3Csvg viewBox='0 0 16 16' fill='none' xmlns='http://www.w3.org/2000/svg'%3E%3Cpath d='M4 6l4 4 4-4' stroke='%23475569' stroke-width='1.4' stroke-linecap='round'/%3E%3C/svg%3E") no-repeat right 8px center;
...
.plan-card {
  background: #13131F; border: 1px solid rgba(255,255,255,0.08); border-radius: 16px; padding: 24px;
...
.mf-input, .mf-select, .mf-textarea {
  padding: 9px 12px; background: #0F0F1A; border: 1px solid rgba(255,255,255,0.1);
...
.btn-ghost {
  padding: 9px 16px; background: none; color: #475569;
  border: 1px solid rgba(255,255,255,0.1); border-radius: 9px;
